...
- Dependencies stay minimal: `leaflet`, `leaflet.markercluster`, `@turf/boolean-point-in-polygon` at runtime; `typescript`, `vite`, `vitest`, `@types/leaflet`, `@types/geojson` at dev-time. Do not add other packages without updating this plan.
...
export function mountPanelLeft(
  ...
  store.subscribe((state) => {
    ...
    infoEl.innerHTML = `<h3>${featureLabel(feature)}</h3><p>${describeTemporalStatus(feature, date)}</p>`;
  });
}
```
