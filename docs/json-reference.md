...
One object per world instance, loaded from `worlds/<id>/world.json` (the id comes from the URL path — see `resolveWorldId` in `docs/api-reference.md`).
...
`welcome` is a plain boolean. `true` makes the shared Home view (`HomeView.ts`) the initial screen; the Home view lists every world in the build and has an "Enter map" button that switches to the map. There is no per-world splash content any more.
