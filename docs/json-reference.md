...
| `theme.primary`                 | `string` (`#rgb` or `#rrggbb`)                     | no                                                                | Base accent color for this world's UI. `deriveTheme()` (`src/ui/theme.ts`) computes the darker/lighter shades (`--color-primary-dark`/`--color-primary-light`) from it; `applyBranding()` sets all three as CSS custom properties at bootstrap. If omitted, the default blue accent (`styles.css`) is kept. |
| `plugins.participate`           | `ParticipateConfig \| undefined`                   | no                                                                | See table below. If omitted, the "Participate" button doesn't appear.                                                                                                                                                                                                                                       |
| `systems.time`                  | `boolean`                                          | no                                                                | `false` hides all time/calendar UI (filters panel's Time section, the header's Calendar nav link, Settings' calendar-system row) — for a world with no temporal data. Defaults to `true` (shown) when omitted.                                                                                              |
| `welcome`                       | `WelcomeConfig \| undefined`                       | no                                                                | See table below. If omitted, the map is the initial view, same as before this field existed. If present, a thematic splash (`WelcomeView.ts`) is the initial view instead; the persistent header's Home link always returns to it.                                                              |
| `about`                         | `AboutConfig \| undefined`                         | no                                                                | See table below. If present, adds an "About Us" link to the persistent header, opening a full-screen About view (`AboutView.ts`).                                                                                                                                                                          |
...
| `ctaLabel`  | `string`             | yes      | Button text. Clicking it sets `view` to `'map'`. The header's Home link always routes back here.                                                                                                                                 |
...
| Field   | Type                | Required | Description                                                                                                          |
| ------- | ------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------- |
| `title` | `string`            | yes      | About page heading.                                                                                                      |
| `body`  | `string[]`          | yes      | One or more paragraphs, rendered in order. Plain text (escaped) — no markdown/HTML.                                     |
| `links` | `{ label, url }[]`  | no       | Arbitrary external links (rendered `target="_blank"`), e.g. a contact page or social profile. Omit for no links section. |
